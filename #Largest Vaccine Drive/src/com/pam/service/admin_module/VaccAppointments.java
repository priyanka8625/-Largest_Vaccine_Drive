...
public class VaccAppointments extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					VaccAppointments frame = new VaccAppointments();
					frame.setVisible(true);
					VaccAppointments.setVaccAppointments();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//		setExtendedState(getExtendedState()|JFrame.MAXIMIZED_BOTH);
		...
		JLabel title = new JLabel("Vaccine Appointment Details");
		title.setFont(new Font("Monospaced", Font.BOLD | Font.ITALIC, 40));
		title.setBounds(10, 38, 703, 65);
		panel.add(title);
	...
	private static void setVaccAppointments() {
		AdminDatabase admin = new AdminDatabase();
		ResultSet rs = admin.getVaccAppointments();
		try {
